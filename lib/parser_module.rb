...
#
# Official Output Structure specs can be found here:
...
#   - Relies on CarInfo::convert_class and CarInfo::convert_drive to convert these integer values to readable strings.
#   - Relies on CarInfo:car_lookup to convert the Car Ordinal integer to an array of readable strings from the
#   CarInfo::CAR_LIST lookup hash:
...
module FMDOParser # rubocop:disable Metrics/ModuleLength
  ...
  #
  # Additional format info: https://ruby-doc.org/3.2.2/packed_data_rdoc.html#label-Integer+Directives
  ...
  # FMDOListener sends this hash as a JSON object to connected websocket clients (browser dashboard), 60/sec
  # - Some values from the full list are instead handled by extract_static_values, and are commented out for clarity
  ...
  # FMDOListener conditionally sends this data to websocket clients once when is_race_on evaluates to true.
  #
  # Relies on CarInfo module to further process Car Ordinal, Car Class, and Drivetrain integers into readable values
  # for display in browser dashboard.
  def extract_static_values(data)
    {
      "Car Ordinal": car_lookup(parse_dashdata(data, 212, 4, 'l')), # converted to array of strings from lookup hash
      "Car Class": convert_class(parse_dashdata(data, 216, 4, 'l')), # converted to readable string
